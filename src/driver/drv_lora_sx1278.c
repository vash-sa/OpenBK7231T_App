...
void LoRa_SendDiscovery(int id) {
    char t[128], p[512];
    const char *dev = ",\"dev\":{\"ids\":[\"lora_%d\"],\"name\":\"LoRa Node %d\"}}";

    // Температура
    sprintf(t, "homeassistant/sensor/lora_%d_t/config", id);
    sprintf(p, "{\"name\":\"Temp\",\"stat_t\":\"lora/%d/s\",\"val_tpl\":\"{{value_json.t}}\",\"unit_of_meas\":\"°C\",\"dev_cla\":\"temperature\"%s", id, id, id);
    MQTT_Publish(t, p, 0, 1);

    // Батарея
    sprintf(t, "homeassistant/sensor/lora_%d_v/config", id);
    sprintf(p, "{\"name\":\"Batt\",\"stat_t\":\"lora/%d/s\",\"val_tpl\":\"{{value_json.v}}\",\"unit_of_meas\":\"V\",\"dev_cla\":\"voltage\"%s", id, id, id);
    MQTT_Publish(t, p, 0, 1);

    // Газ
    sprintf(t, "homeassistant/sensor/lora_%d_p/config", id);
    sprintf(p, "{\"name\":\"Gas\",\"stat_t\":\"lora/%d/s\",\"val_tpl\":\"{{value_json.p}}\",\"unit_of_meas\":\"ppm\"%s", id, id, id);
    MQTT_Publish(t, p, 0, 1);

    // Дым (Бинарный сенсор)
    sprintf(t, "homeassistant/binary_sensor/lora_%d_s/config", id);
    sprintf(p, "{\"name\":\"Smoke\",\"stat_t\":\"lora/%d/s\",\"val_tpl\":\"{{'ON' if value_json.s=='YES' else 'OFF'}}\",\"dev_cla\":\"smoke\"%s", id, id, id);
    MQTT_Publish(t, p, 0, 1);
}
