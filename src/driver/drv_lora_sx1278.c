...
void LoRa_SendDiscovery(int id) {
    ...
    // Регистрируем ТОЛЬКО ОДИН топик (основной - Дым)
    snprintf(t, sizeof(t), "homeassistant/binary_sensor/lora_%d", id);

    // Формируем один жирный пакет
    snprintf(p, sizeof(p), 
        "{"
        "\"name\":\"Датчик %d\","
        "\"stat_t\":\"lora/%d\","
        "\"val_tpl\":\"{{'ON' if value_json.s=='YES' else 'OFF'}}\","
        "\"dev_cla\":\"smoke\","
        "\"uniq_id\":\"l_%d_main\","
        "\"json_attr_t\":\"lora/%d\"," // Откуда брать атрибуты
        "\"json_attr_tpl\":\"{{ {'t': value_json.t, 'v': value_json.v, 'g': value_json.g} | tojson }}\"," // Прописываем ключи
        "\"dev\":{\"ids\":[\"l_node_%d\"],\"name\":\"Пожарный датчик %d\",\"mdl\":\"LoRa-V1\"}"
        "}", 
        id, id, id, id, id, id);
...
void LoRa_SendDiscovery(int id) {
    ...
    // Общая часть для всех сенсоров (устройство)
    ...
    len = snprintf(p, sizeof(p), 
        "{"
            ...
            "\"uniq_id\":\"l_%d_s\""
        "}", id, id);
    // Удаляем закрывающую скобку '}', чтобы дописать dev_json
    if (len > 0) p[len-1] = '\0'; 
    snprintf(p + strlen(p), sizeof(p) - strlen(p), dev_json, id, id);
    MQTT_Publish(t, "config", p, 3);
...
void LoRa_SendDiscovery(int id) {
    ...
    snprintf(p, sizeof(p), "{\"name\":\"Заряд\",\"stat_t\":\"lora/%d\",\"val_tpl\":\"{{value_json.v}}\","
        "\"unit_of_meas\":\"V\",\"dev_cla\":\"voltage\",\"uniq_id\":\"l_%d_v\"", id, id);
...
void LoRa_SendDiscovery(int id) {
    ...
    // Группировка: параметр "name" в dev — это заголовок всей карточки в HA
    const char* dev = "\"dev\":{\"ids\":[\"l_node_%d\"],\"name\":\"Пожарный датчик %d\"}";
    ...
    snprintf(p, sizeof(p), 
        "{\"name\":\"Заряд\",\"stat_t\":\"lora/%d\",\"val_tpl\":\"{{value_json.v}}\","
        "\"unit_of_meas\":\"V\",\"dev_cla\":\"voltage\",\"uniq_id\":\"l_%d_v\","
        "\"ic\":\"mdi:battery\",\"sug_dsp_prc\":1,\"obj_id\":\"l_%d_v\",", 
        id, id, id);    
    sprintf(p + strlen(p), dev, id, id);
    strcat(p, "}");
    MQTT_Publish(t, "config", p, 3);
    ...
    snprintf(p, sizeof(p), "{\"name\":\"Дым\",\"stat_t\":\"lora/%d\",\"val_tpl\":\"{{'ON' if value_json.s=='YES' else 'OFF'}}\",\"dev_cla\":\"smoke\",\"uniq_id\":\"l_%d_s\",\"obj_id\":\"l_%d_s\",", id, id, id);
    sprintf(p + strlen(p), dev, id, id);
    strcat(p, "}");
    MQTT_Publish(t, "config", p, 3);
    ...
    snprintf(p, sizeof(p), "{\"name\":\"CO\",\"stat_t\":\"lora/%d\",\"val_tpl\":\"{{value_json.g}}\",\"unit_of_meas\":\"ppm\",\"dev_cla\":\"co2\",\"uniq_id\":\"l_%d_g\",\"obj_id\":\"l_%d_g\",", id, id, id);
    sprintf(p + strlen(p), dev, id, id);
    strcat(p, "}");
    MQTT_Publish(t, "config", p, 3);
}*/
